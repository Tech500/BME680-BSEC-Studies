/*
** 
**                      BME680 BSEC Gas Study  3/12/2021 @ 08:52 EST  --ESPAsyncWebserver version
**                      William Lucid
**
...
//int interval -- time between dataPoints
//int wait; //Number of datapoints to wait --pre-heating BME680
...
/*
    ...
    This is the ThingSpeak channel number for the MathwWorks weather station
     ...
     Field 4 - Gas Resistance  (KOhm)            
     Field 5 - IAQ (Indoor Air Qualitiy)
	   Field 6 - CO2 (KOhm)
     Field 7 - VOC (KOhm)
...
void setup()
{
     ...
     Serial.println(F("\n\n\nBME680 BSEC Gas Study  3/12/2021 @ 08:52 EST"));
  ...
  checkIaqSensorStatus();
     ...
     Serial.println("\n\nAllow 15 dataPoints (30 Minutes) before starting Test");
...
void loop()
{
    ...
    {     
        ...
        if(dataPoint == wait) //default = 15 (30 minutes)
        {
...
void thingSpeak()
{
    ...
    dtostrf(temperatureF, 6, 1, fltemperatureF);
    char flhumidity[15];
    dtostrf(humidity, 5, 1, flhumidity);
    char flpressure[15];
    dtostrf(baro, 6, 3, flpressure);
    char flgas[15];
    dtostrf(gasResistance,7, 3, flgas);
    char fliaq[15];
    dtostrf(sensor,5, 3, fliaq);
    char flCO2[15];
    dtostrf((gasCO2), 7, 3, flCO2);
    char flVOC[15];
    dtostrf((gasVOC), 7, 3, flVOC);
     ...
     ThingSpeak.setField(5, fliaq);
     ThingSpeak.setField(6, flCO2);
     ThingSpeak.setField(7, flVOC);
